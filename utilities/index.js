...
Util.buildItemGrid = async function(data) {
  ...
  if (itemAmount.length === 1) {
    grid += '<div class="single-item-display">';
    grid += '<div class="card">';
    grid += '<img src="' + vehicle[0].inv_thumbnail + '" alt="Image of ' + vehicle[0].inv_make + ' ' + vehicle[0].inv_model + ' on CSE Motors" />';
    grid += '</div>';
    grid += '<div class="single-item-details">';
    grid += '<h3 class="this-title">' + (vehicle[0].inv_make + ' ' + vehicle[0].inv_model) + ' Details</h3>';
    grid += '<h4 class="price"><strong>Price:</strong> $' + new Intl.NumberFormat('en-US').format(vehicle[0].inv_price) + '</h4>';
